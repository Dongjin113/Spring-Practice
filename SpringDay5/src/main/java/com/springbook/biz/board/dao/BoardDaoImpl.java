...
public class BoardDaoImpl implements BoardDao {
	...
	private final String UPDATE_SQL="update board set  title=?,  content=? where seq=?   ";
	...
	public List<BoardVo> getBoardList(BoardVo vo) {
		System.out.println("SearchKeyword:" + vo.getSearchKeyword() + " ,SearchCondition:"  + vo.getSearchCondition());
		
		if (vo.getSearchKeyword() == null || vo.getSearchCondition().equals("") ) {
		  return  jdbcTemplate.query(GETBOARDLIST_SQL, new BoardRowMapper());
		} else if(vo.getSearchKeyword().equals("title")) {
		  return  jdbcTemplate.query(GETBOARDTITLE_SQL, new BoardRowMapper(), "%" + vo.getSearchCondition() + "%");
		} else if(vo.getSearchKeyword().equals("writer")) {
		  return  jdbcTemplate.query(GETBOARDWRITER_SQL, new BoardRowMapper(), "%" + vo.getSearchCondition() + "%");
		}		
		return null;
